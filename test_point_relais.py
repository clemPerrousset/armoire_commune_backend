...
from test_app import get_session_override, engine
...
def test_signup_defaults_to_false_point_relais(client: TestClient, session: Session):
    ...
    data = response.json()
    assert data["is_point_relais"] is False

    # Also verify in DB
    user = session.exec(select(User).where(User.email == "new@example.com")).first()
    assert user.is_point_relais is False

def test_admin_can_promote_point_relais(client: TestClient, session: Session):
...
def test_normal_user_cannot_promote_point_relais(client: TestClient, session: Session):
    user1 = create_user(session, "user1@example.com")
    user2 = create_user(session, "user2@example.com")

    user1_token = get_token(client, user1.email)
...
def test_get_users_me_returns_point_relais(client: TestClient, session: Session):
